...
const refParser = async (ref) => {
  ...
  const dvc_checks = checks.data.check_runs.filter(check => {  console.log([check.name, CI.DVC_TITLE]); console.log(check.name === CI.DVC_TITLE); return check.name === CI.DVC_TITLE}  );
